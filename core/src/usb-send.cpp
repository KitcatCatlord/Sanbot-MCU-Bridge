...
void SanbotUsbManager::sendToHead(const vector<uint8_t> &frame) {
  enqueueMessage(WHAT_SEND_TO_HEAD, frame);
}

void SanbotUsbManager::sendToBottom(const vector<uint8_t> &frame) {
  enqueueMessage(WHAT_SEND_TO_BOTTOM, frame);
}

void SanbotUsbManager::sendToPoint(const vector<uint8_t> &routedFrameWithTag) {
  enqueueMessage(WHAT_SEND_TO_POINT, routedFrameWithTag);
}

void SanbotUsbManager::enqueueMessage(int what, const vector<uint8_t> &data) {
...
void SanbotUsbManager::handlePointMessage(const vector<uint8_t> &buffers) {
  if (buffers.size() < 2)
    return;
...
void SanbotUsbManager::sendBufferTo(EndpointSet &dev, uint16_t pid,
  ...
  int transferred = 0;
  int r = libusb_bulk_transfer(dev.handle, dev.outEp,
                               const_cast<unsigned char *>(buf.data()),
                               static_cast<int>(buf.size()), &transferred, 0);
