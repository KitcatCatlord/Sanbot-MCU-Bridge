...
This project originally started as a Python library intended to control Sanbot’s MCUs. I later decided to rewrite it in C++ for the reasons explained below. The original repository has now been archived as Sanbot-MCU-Bridge-Old, and a new repository was created since the old one had become quite messy.

## Why the rewrite?

The project was originally written in Python partly because I did not fully know what it would turn into, and partly because I did not think far enough ahead at the time. The Python prototype mixed hand-written code with fast experimental scaffolding while I was learning the firmware behavior.
...
Python was slow and the original library was buggy and dysfunctional. Rewriting everything in C++ gives me a clean slate and the chance to do it properly now that I understand the system much better.
